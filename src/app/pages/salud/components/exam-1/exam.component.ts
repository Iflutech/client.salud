...
export class ExamContentComponent {
  ...
  constructor(
    ...
    let base64String = localStorage.getItem('sheet_sequence')
    if (base64String == null) {
      this.emitLogout.emit()
    } else {
      this.decodeBase64(base64String)//OBTENER DATOS PERSONALES Y TIPO EXAMEN DESDE LOCAL STORAGE 
    ...
    this.asignarDatos(this.decodedData)
  }
  ...
  registerExam(): void {
    ...
    if(this.validateDataExam()) {
      setTimeout(() => {
        this.isLoadingData = false
        let datos = this.transformExamsData(this.exams);
        // console.log(datos);
  ...
  transformExamsData(exams: Exam[]): TransformedData[] {
    return exams.map((exam) => ({
      category: exam.category,
      answer: exam.questions.map((question) => {
        const selectedResponse = question.responses.find(
          (response) => response.value === question.selectAnswer
        )
        return {
          seq: question.seq,
  ...
  asignarDatos(data: string[]): void {
    this.formHeaderData.get('dni')!.setValue(data[0])
    this.formHeaderData.get('name')!.setValue(data[1])
    this.formHeaderData.get('operation')!.setValue(data[2])
    this.formHeaderData.get('job')!.setValue(data[3])
    this.formHeaderData.get('system')!.setValue(data[4])
...
// EJEMPLO PARA MANDAR DATOS AL BACKEND
